...
def iso_run():
    ...
    for tpc in accelerator_list:
        ...
        if vdp_type=='HQNNA':
            ...
            sconna_unit_power = dpe_obj.input_actuation_power*osm_bank_mrrs + dac_obj.power*no_of_dacs*mW_to_W + adc_obj.power*no_of_adc*mW_to_W + soa_obj.power*no_of_soas*mW_to_W + pd_obj.power*no_of_pds*mW_to_W + laser_power*wall_plug_efficiency + BtoS_obj.power*(dpe_count/8)*mW_to_W
            ...
            no_of_pds = dpe_count
            no_of_adc = dpe_count  
            laser_power = laser_power_per_wavelength*25 #! The number of lasers in HSCONNA are fixed to 25 wavelength based on the comb source. 
            hsconna_unit_power = dpe_obj.input_actuation_power*osm_bank_mrrs + dac_obj.power*no_of_dacs*mW_to_W + adc_obj.power*no_of_adc*mW_to_W + soa_obj.power*no_of_soas*mW_to_W + pd_obj.power*no_of_pds*mW_to_W + laser_power*wall_plug_efficiency + BtoS_obj.power*dpe_count*mW_to_W
            
            print('HSCONNA Unit Power', hsconna_unit_power)
            total_hsconna_unit_power = hsconna_unit_power*dpu_count 
            total_power = total_hsconna_unit_power
            ...
            # LIGHTBuLB unit static power
            weight_bank_mrrs = 3*dpe_count*dpe_size
            input_bank_mrrs = dpe_size
            no_of_pds = dpe_count
            no_of_adc = dpe_count
            no_of_shifters = dpe_count
            laser_power = laser_power_per_wavelength*dpe_size
            no_of_rn = 1
            lightbulb_unit_power = no_of_rn*rn_obj.power*mW_to_W+dpe_obj.weight_actuation_power*no_of_mrrs + wgt_dac_obj.power*weight_bank_mrrs*mW_to_W + act_dac_obj.power*input_bank_mrrs*mW_to_W + adc_obj.power*no_of_adc*mW_to_W + pd_obj.power*no_of_pds*mW_to_W + laser_power*wall_plug_efficiency+no_of_shifters*shifter_obj.power

            print('LIGHTBuLB Unit Power', lightbulb_unit_power)
            total_lightbulb_unit_power = lightbulb_unit_power*dpu_count 
            total_power = lightbulb_unit_power
            print("LIGHTBULB Power", total_power*1e3, "mW")
        elif vdp_type =="OXBNN":
            dpe_obj = MRR_DPE(conv_dpe_size,data_rate)
            rn_obj = RN(reduction_network_type)
            act_dac_obj = DAC_1b()
            ...
            laser_power = laser_power_per_wavelength*dpe_size
            no_of_rn = 1
            oxbnn_unit_power = no_of_rn*rn_obj.power*mW_to_W+dpe_obj.weight_actuation_power*no_of_mrrs + wgt_dac_obj.power*weight_bank_mrrs*mW_to_W + act_dac_obj.power*input_bank_mrrs*mW_to_W + adc_obj.power*no_of_adc*mW_to_W + pd_obj.power*no_of_pds*mW_to_W + laser_power*wall_plug_efficiency

            print('OXBNN Unit Power', oxbnn_unit_power)
            total_oxbnn_unit_power = oxbnn_unit_power*dpu_count 
            total_power = total_oxbnn_unit_power
            print("OXBNN Power", total_power*1e3, "mW") 


def get_static_power(reduction_network_type, vdp_type, data_rate,conv_dpe_count, conv_dpe_size, conv_dpu_count, fc_dpe_count, fc_dpe_size, fc_dpu_count , dpe_count, dpe_size, dpu_count):    
    mW_to_W = 1e-3
    laser_power_per_wavelength = 1.274274986e-3 # W
    wall_plug_efficiency = 5 
    if vdp_type=='HQNNA':
